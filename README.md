# JamalC2

> **现代化轻量级命令与控制框架**  
> 基于 Rust + Tauri 构建

---

## ✨ 特性

| 特性 | 描述 |
|------|------|
| 🔐 **端到端加密** | ChaCha20-Poly1305 认证加密 |
| 🌐 **HTTP Beacon** | 轻量级 HTTP 轮询通信 |
| 🎭 **流量伪装** | 随机 User-Agent + 伪装业务字段 |
| ⏱️ **动态心跳** | 可调节间隔 + 自动 ±20% 抖动 |
| 🖥️ **现代 GUI** | React + Tauri 跨平台界面 |
| 📦 **轻量 Implant** | Rust (~1MB) / C (~100KB) 双版本 |
| 🛡️ **规避能力** | 反沙箱检测 + 动态API解析 + 睡眠混淆 |
| 🐧 **跨平台 Server** | Windows (Tauri) / Linux (Docker) |

---

## 🏗️ 架构

```
┌─────────────┐    HTTPS/HTTP    ┌──────────────┐
│   Implant   │ ◄──────────────► │   C2 Server  │
│  (Windows)  │   ChaCha20 加密   │  (Tauri App) │
└─────────────┘                  └──────────────┘
```

---

##  功能列表

### 已实现

| 功能 | 描述 |
|------|------|
| ✅ Shell 执行 | 远程命令执行，30秒超时 |
| ✅ 文件管理 | 浏览/上传/下载/删除 |
| ✅ 心跳调节 | 实时修改 Beacon 间隔 |
| ✅ Jitter 抖动 | >10秒自动 ±20% 随机化 |
| ✅ 断开连接 | 远程终止 Implant |
| ✅ Implant 生成 | GUI 一键生成 Rust/C 双版本 |
| ✅ 流量伪装 | 伪装 Header + Body 字段 |
| ✅ 启动密钥 | `-k <key>` 验证 + 自删除 |
| ✅ 反沙箱检测 | 进程数/内存/硬盘/用户名/睡眠加速检测 |
| ✅ 动态 API 解析 | 哈希混淆 + PEB 遍历，规避静态分析 |
| ✅ 睡眠混淆 | 睡眠期间加密敏感数据 |

### 路线图

| 功能 | 优先级 |
|------|:------:|
| 屏幕截图 | P1 |
| 进程管理 | P1 |
| 键盘记录 | P2 |
| 持久化 | P2 |

---

## 📦 安装

### 环境要求

| 依赖 | 说明 |
|------|------|
| **Visual Studio Build Tools** | C++ 编译工具（必须手动安装） |
| **Rust** | 1.70+ |
| **Node.js** | 18+ |

---

### 第一步：安装 Visual Studio Build Tools（必须）

> ⚠️ **重要**：这一步必须手动完成，自动安装通常无法正确配置 C++ 组件。

1. 访问 https://visualstudio.microsoft.com/visual-cpp-build-tools/
2. 点击 **"下载 Build Tools"**
3. 运行下载的安装程序
4. 在安装界面中，**勾选 "Desktop development with C++"**（使用 C++ 的桌面开发）
5. 点击 **"安装"**，等待完成（约 10-20 分钟）
6. **重启电脑**

#### 验证安装

打开 **"Developer Command Prompt for VS"**（开始菜单搜索），运行：

```cmd
cl.exe
```

如果显示版本信息，说明安装成功。

---

### 第二步：安装 Rust 和 Node.js

```powershell
# 安装 Rust
winget install Rustlang.Rustup

# 安装 Node.js
winget install OpenJS.NodeJS.LTS
```

安装后**重启终端**。

---

### 第三步：克隆项目并运行

```powershell
# 克隆项目
git clone https://github.com/zalehuoer/jamal.git
cd jamal

# 运行安装脚本（自动安装 npm 依赖）
.\install.bat

# 或手动安装依赖
cd server
npm install
```

---

## 🐧 Linux 服务端部署（Docker）

适用于部署到 Linux VPS/云服务器的无头环境。

### 方式一：一键部署

```bash
# 克隆项目
git clone https://github.com/zalehuoer/jamal.git
cd jamal

# 运行部署脚本
chmod +x deploy.sh
./deploy.sh
```

### 方式二：手动 Docker 部署

```bash
# 构建镜像
docker compose build

# 启动服务
docker compose up -d

# 查看日志
docker compose logs -f

# 停止服务
docker compose down
```

### 配置端口

编辑 `docker-compose.yml`：

```yaml
ports:
  - "4444:4444"  # 修改左边的端口号
```

---

## 🚀 快速开始

### 1. 启动 Server

```bash
cd server
npm run tauri dev
```

### 2. 创建监听器

**设置** → 输入端口 → **创建并启动**

### 3. 生成 Implant

**生成** → 选择监听器 → 填写地址 → **生成**

### 4. 部署

```powershell
.\implant.exe -k <key>
```

---

## 🌐 使用 HTTPS（通过 ngrok）

### 1. 安装 ngrok

访问 https://ngrok.com/download 下载并安装。

### 2. 启动 ngrok 转发

```powershell
# 确保在运行 Server 的同一台机器上执行
ngrok http 127.0.0.1:4444
```

ngrok 会显示类似这样的地址：
```
Forwarding  https://abc123.ngrok-free.app -> http://localhost:4444
```

### 3. 生成 Implant（使用 ngrok 地址）

| 配置项 | 值 |
|--------|-----|
| 服务器地址 | `abc123.ngrok-free.app`（不带 https://） |
| 端口 | `443` |
| 使用 HTTPS | ✅ 勾选 |

### 4. 部署 Implant

```powershell
.\implant.exe -k <key>
```

> ⚠️ **注意**：ngrok 和 Server 必须在**同一台机器**上运行，否则会出现 502 错误。

---

## 🔒 安全机制

### 加密

- **算法**: ChaCha20-Poly1305 (AEAD)
- **密钥**: 256-bit
- **Nonce**: 12 字节随机

### 流量伪装

**请求示例**:
```json
{
  "apiVersion": "2.1.0",
  "clientId": "a1b2c3d4-...",
  "authToken": "Bearer 8f2e4a...",
  "requestId": "req_1a2b3c...",
  "timestamp": 1703318400,
  "platform": "Windows",
  "data": "加密数据..."
}
```

**Header 伪装**:
- 随机 User-Agent (Chrome/Firefox/Windows Update)
- X-Request-ID, X-Client-Version 等

### 运行保护

- 需要 `-k <key>` 参数启动
- 参数错误自动删除自身

---

## 📁 项目结构

```
jamalC2/
├── server/           # Windows 控制端 (Tauri + React)
├── server-web/       # Linux 控制端 (Web API)
├── implant/          # Rust Implant
├── implant-c/        # C Implant (更小体积 + 规避能力)
└── shared/           # 共享库 (加密 + 消息)
```

---

## ⚙️ 技术栈

| 组件 | 技术 |
|------|------|
| Server 前端 | React + TypeScript |
| Server 后端 | Rust + Tauri + Axum |
| Implant | Rust (ureq + native-tls) |
| 加密 | chacha20poly1305 |
| 序列化 | serde + bincode |

---

## 📝 更新日志

### v1.1.0 (2024-12)

- ✅ **C Implant** - 轻量级 C 版本 (~100KB)
- ✅ **反沙箱检测** - 6 项检测 + 评分系统
- ✅ **动态 API 解析** - DJB2 哈希 + PEB 遍历
- ✅ **睡眠混淆** - 睡眠期间加密敏感数据
- ✅ **Linux 服务端** - Docker 一键部署
- ✅ **Implant 生成器** - 支持 Rust/C 双版本

### v1.0.0 (2024-12)

- ✅ ChaCha20-Poly1305 加密通信
- ✅ HTTP Beacon 轮询
- ✅ 流量伪装（Header + Body）
- ✅ 启动密钥 + 自删除
- ✅ 心跳调节 + Jitter 抖动
- ✅ 体积优化（1.5MB → 1MB）
- ✅ 同步架构重构（ureq 替代 reqwest）

---

## ⚠️ 免责声明

本工具**仅供安全研究和授权测试使用**。未经授权对计算机系统进行攻击属于违法行为。使用者需自行承担法律责任。
